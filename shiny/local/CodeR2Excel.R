xlsx.addHeader<-function(wb, sheet, value="Header", level=1, color="#FFFFFF",
                         startRow=NULL, startCol=2, underline=c(0,1,2),backGroundColor="white"){
	library("xlsx")
	...
	if(color=="black") color="white"# black and white color are inversed in xlsx package. don't know why
  	...
  	H1_STYLE <- CellStyle(wb) + Font(wb,  heightInPoints=22,color=color, isBold=TRUE, underline=underline[1]) #+ Fill(backgroundColor=backGroundColor, foregroundColor=backGroundColor)
	H2_STYLE <- CellStyle(wb) + Font(wb, heightInPoints=18, color=color, isItalic=FALSE, isBold=TRUE, underline=underline[1]) + Fill(backgroundColor=backGroundColor, foregroundColor=backGroundColor)
  	H3_STYLE <- CellStyle(wb) + Font(wb, heightInPoints=16, color=color, isItalic=TRUE, isBold=TRUE, underline=underline[1]) + Fill(backgroundColor=backGroundColor, foregroundColor=backGroundColor)
  	H4_STYLE <- CellStyle(wb) + Font(wb, heightInPoints=16, color=color, isItalic=TRUE, isBold=FALSE, underline=underline[1]) + Fill(backgroundColor=backGroundColor, foregroundColor=backGroundColor)
  	H5_STYLE <- CellStyle(wb) + Font(wb, heightInPoints=14, color=color, isItalic=TRUE, isBold=FALSE, underline=underline[1]) + Fill(backgroundColor=backGroundColor, foregroundColor=backGroundColor)
  	H6_STYLE <- CellStyle(wb) + Font(wb, heightInPoints=12, color=color, isItalic=TRUE, isBold=TRUE, underline=underline[2]) + Fill(backgroundColor=backGroundColor, foregroundColor=backGroundColor)
	H66_STYLE <- CellStyle(wb) + Font(wb, heightInPoints=12, color=color, isItalic=TRUE, isBold=TRUE, underline=underline[2])
	...
	else if(level==66) xlsx::setCellStyle(sheetTitle[[1,1]], H66_STYLE)  
}


xlsx.addParagraph<-function(wb,sheet, value, fontColor="#FFFFFF", fontSize=12, backGroundColor="#FFFFFF",
                            isBold=FALSE, isItalic=FALSE, startRow=NULL, startCol=2, colSpan=10, rowSpan=5){
	library("xlsx") 
...
xlsx.addHyperlink<-function(wb,sheet, address, friendlyName, fontColor="blue", fontSize=12,
...
xlsx.addTable<-function(wb, sheet, data, startRow=NULL,startCol=1, col.names=TRUE, row.names=TRUE, columnWidth=14,
                        fontColor="#FFFFFF", fontSize=12, rownamesFill="white", colnamesFill="white", 
                        rowFill="white",whichRows=NULL,checksIndex=c()){ #
	library("xlsx")
  	#++++++++++++++++++++++++++++++++++++++
  	#Define table style
  	#++++++++++++++++++++++++++++++++++++++
  	...
  	column.style=NULL
  	for(i in 1:col.n){
  		column.style[[i]]=CellStyle(wb, font=Font(wb, color=fontColor, heightInPoints=fontSize)) + Alignment(horizontal="ALIGN_CENTER")
 	}
  	...
  	if(!rowFill=="white"){
      		col.n =ncol(data)
      		row.n=nrow(data)
      		if(col.names==TRUE) row.n<-row.n+1
    		if(row.names==TRUE) col.n<-col.n+1
      		cb<-CellBlock(sheet, startRow=startRow, startColumn=startCol, noRows=row.n, noColumns=col.n, create=FALSE )
      		#color pair row for styling
		for(i in 1: nrow(data)){ 
			if(whichRows[i]){
				if(col.names==TRUE){
					CB.setFill( cb, fill=Fill(foregroundColor = rowFill, backgroundColor=rowFill), rowIndex=i+1, colIndex=1:col.n)
				}else{
					CB.setFill( cb, fill=Fill(foregroundColor = rowFill, backgroundColor=rowFill), rowIndex=i, colIndex=1:col.n)
				}
			}
    		}
   	}
	if(length(checksIndex) > 0){
      		col.n =ncol(data)
      		row.n=nrow(data)
      		if(col.names==TRUE) row.n<-row.n+1
    		if(row.names==TRUE) col.n<-col.n+1
      		cb<-CellBlock(sheet, startRow=startRow, startColumn=startCol, noRows=row.n, noColumns=col.n, create=FALSE )
      		#color pair row for styling
		for(i in 1:length(checksIndex)){ 
			if(col.names==TRUE){
				CB.setFill( cb, fill=Fill(foregroundColor = "yellow", backgroundColor="yellow"), rowIndex=checksIndex[i]+1, colIndex=1:col.n)
			}else{
				CB.setFill( cb, fill=Fill(foregroundColor = "yellow", backgroundColor="yellow"), rowIndex=checksIndex[i], colIndex=1:col.n)			
			}
    		}
   	}
}
...
xlsx.addTableBlocks<-function(wb, sheet, data, startRow=NULL, startCol=1, col.names=TRUE, row.names=TRUE, columnWidth=14,
                        fontColor="#FFFFFF", fontSize=12, rownamesFill="white", colnamesFill="white", 
                        rowFill="white",nband=NULL, nstack=NULL){ #
	library("xlsx")
  	#++++++++++++++++++++++++++++++++++++++
  	#Define table style
  	#++++++++++++++++++++++++++++++++++++++
  	...
  	if(rownamesFill!="white") {
  		TABLE_ROWNAMES_STYLE <- TABLE_ROWNAMES_STYLE+
                	Fill(foregroundColor = rownamesFill, backgroundColor=rownamesFill)
  	}
                
  
  	TABLE_COLNAMES_STYLE <- CellStyle(wb) + Font(wb, isBold=TRUE, color=fontColor, heightInPoints=fontSize) +
    			Alignment(wrapText=TRUE, horizontal="ALIGN_CENTER") + Border(color="black", position=c("TOP", "BOTTOM"), 
        		pen=c("BORDER_THIN", "BORDER_THICK"))

  	#colnames fill
  	if(colnamesFill!="white") {
  		TABLE_COLNAMES_STYLE <-TABLE_COLNAMES_STYLE+
                	Fill(foregroundColor = colnamesFill, backgroundColor=colnamesFill)
  	}
  	...
  	for(i in 1:col.n){
  		column.style[[i]] = CellStyle(wb, font=Font(wb, color=fontColor, heightInPoints=fontSize)) + Alignment(horizontal="ALIGN_CENTER") +
			Border(color="black", position=c("LEFT", "RIGHT", "BOTTOM"), pen=c("BORDER_DOTTED", "BORDER_DOTTED","BORDER_DOTTED"))
 	}
  	names(column.style) <- as.character(1:ncol(data))
	...
	if(!rowFill=="white"){
		for(i in 1: nrow(data)){ 
			if(col.names==TRUE){
				CB.setFill( cb, fill=Fill(foregroundColor = rowFill, backgroundColor=rowFill), rowIndex=i+1, colIndex=1+1:col.n)
			}else{
				CB.setFill( cb, fill=Fill(foregroundColor = rowFill, backgroundColor=rowFill), rowIndex=i, colIndex=1+1:col.n)
			}
    		}
   	}
	...
	for(i in 1: nrow(data)){ 
		for(j in 1: ncol(data)){
			if(ceiling(j/nstack)%%2 == ceiling(i/nband)%%2){
				colorFill <- "yellow"
			}else{
				colorFill <- "green"
			}
			if(col.names==TRUE){
				CB.setFill( cb, fill=Fill(foregroundColor = colorFill, backgroundColor=colorFill), rowIndex=i+1, colIndex=j+1) 
			}else{
				CB.setFill( cb, fill=Fill(foregroundColor = colorFill, backgroundColor=colorFill), rowIndex=i, colIndex=j+1)
			}
		}
    	}
}
...
xlsx.addTableSatisfy<-function(wb, sheet, data, startRow=NULL,startCol=1, col.names=TRUE, row.names=TRUE, columnWidth=14,
                        fontColor="#FFFFFF", fontSize=12, rownamesFill="white", colnamesFill="white", 
                        rowFill="white",checks=0){ #
	library("xlsx")
  	#++++++++++++++++++++++++++++++++++++++
  	#Define table style
  	#++++++++++++++++++++++++++++++++++++++
	...
	if( checks != 0 ){
		...
		for(i in 1:nrow(data)){ 
			if(col.names==TRUE){
				CB.setFill( cb, fill=Fill(foregroundColor = "yellow", backgroundColor="yellow"), rowIndex=i+1, colIndex=1:col.n)
			}else{
				CB.setFill( cb, fill=Fill(foregroundColor = "yellow", backgroundColor="yellow"), rowIndex=i, colIndex=1:col.n)			
			}
    		}
   	}
}
...
xlsx.addTableSxLxT<-function(wb, sheet, data, startRow=NULL,startCol=1, col.names=TRUE, row.names=TRUE, columnWidth=14,
                        fontColor="#FFFFFF", fontSize=12, rownamesFill="white", colnamesFill="white", 
                        rowFill="white",values=1:6){ #
	library("xlsx")
  	#++++++++++++++++++++++++++++++++++++++
  	#Define table style
  	#++++++++++++++++++++++++++++++++++++++
  	...
  	if(!rowFill=="white"){
		...
		heatcols <- heat.colors(length(values))

		for(i in 1:nrow(data)){ 
			for(j in 1:ncol(data)){
				CB.setFill( cb, fill=Fill(foregroundColor = heatcols[which(data[i,j]==values)],backgroundColor=heatcols[which(data[i,j]==values)]), rowIndex=i+1, colIndex=j+1)
			}
    		}
   	}
}
...
xlsx.addPlot<-function( wb, sheet, plotFunction, startRow=NULL,startCol=2, width=480, height=480,... ){
  	library("xlsx")
...
xlsx.writeFile<-function(data, file, sheetName="Sheet1", col.names=TRUE, row.names=TRUE, append=FALSE, ...){
