genSPPSB<-function(rowsN,colsN,PbS,Sm,Km,dataCc,C,dataNCc,Z1,ng,Z2_q,Q,B,N,R,name,sumdat){
  library(xlsx)
  library(rfunctions)
  library(Matrix)
  library(MASS)
  ...
  for( j in 1:(Q-2)){
    if(Z2_q<ng){
      nng=which(unlist(lapply(split(datatmp$IDnum,datatmp$Group),length))>0)
      if(length(nng)>=Z2_q){
        selgrp=sample(nng,Z2_q)
        
        unosel=which(unlist(lapply(split(datatmp$IDnum,datatmp$Group)[selgrp],length))==1)
        if(length(unosel)!=0){
          unoselgrp=unlist(split(datatmp$IDnum,datatmp$Group)[selgrp[unosel]])
          data_splitZ[[j+1]]=c(unoselgrp,unlist(lapply(split(datatmp$IDnum,datatmp$Group)[selgrp[-unosel]],function(x) sample(x,1))))
        }else{
          data_splitZ[[j+1]]=unlist(lapply(split(datatmp$IDnum,datatmp$Group)[selgrp],function(x) sample(x,1)))
        }
        ...
        data_splitZ[[j+1]]=sample(unlist(split(datatmp$IDnum,datatmp$Group)[nng]),Z2_q)
        dataNC$SetNum[data_splitZ[[j+1]]]=paste("Set",j+1,sep="")
        datatmp=datatmp[-which(datatmp$IDnum%in%data_splitZ[[j+1]]==TRUE),]
        setname=c(paste("Set",j+1,sep=""),setname)
      }
    ...
    if(Z2_q%%ng==0){
      ...
      Z2ng=floor(Z2_q/ng)
      rest2=Z2_q-(ng*Z2ng)
      samrest=sample(datatmp$IDnum,rest2)
      datatmp=datatmp[-which(datatmp$IDnum%in%samrest==TRUE),]
    }
  
    if((Z2ng)!=1){
      NOempty=lapply(split(datatmp$IDnum,datatmp$Group),length)>=Z2ng
    }else{
      NOempty=lapply(split(datatmp$IDnum,datatmp$Group),length)>Z2ng
    }
    if(all(NOempty)==TRUE){
      if(Z2_q%%ng==0){
        data_splitZ[[j+1]]=unlist(lapply(split(datatmp$IDnum,datatmp$Group),function(x) sample(x,Z2ng)))
      }else{
        data_splitZ[[j+1]]=c(samrest,unlist(lapply(split(datatmp$IDnum,datatmp$Group),function(x) sample(x,Z2ng))))
      }
      dataNC$SetNum[data_splitZ[[j+1]]]=paste("Set",j+1,sep="")
      datatmp=datatmp[-which(datatmp$IDnum%in%data_splitZ[[j+1]]==TRUE),]
      setname=c(paste("Set",j+1,sep=""),setname)
    }else{
      fillitems=which(NOempty==FALSE)
      zerodata=unlist(split(datatmp$IDnum,datatmp$Group)[fillitems])
      if(length(zerodata)!=0){
        datatmp2=datatmp[-which(datatmp$IDnum%in%zerodata==TRUE),] 
      }else{
        datatmp2=datatmp
      }
      datatmp2$Group=as.factor(as.character(datatmp2$Group))
      data_splitmp2=unlist(lapply(split(datatmp2$IDnum,datatmp2$Group),function(x) sample(x,Z2ng)))
      datatmp=datatmp[-which(datatmp$IDnum%in%data_splitmp2==TRUE),]
      
      items=unlist(lapply(split(datatmp$IDnum,datatmp$Group),length))
      nsize=(Z2ng)*length(fillitems)
      need=nsize-sum(items[fillitems])
      maxg=which(items>(Z2ng))
      if(any(items[fillitems]!=0)){
        data_splitmp=unlist(split(datatmp$IDnum,datatmp$Group)[fillitems])
        datatmp=datatmp[-which(datatmp$IDnum%in%data_splitmp==TRUE),]
        if (need!=0){
          datatake=unlist(split(datatmp$IDnum,datatmp$Group)[maxg])
          sampleneed=sample(datatake,need)
          datatmp=datatmp[-which(datatmp$IDnum%in%sampleneed==TRUE),]
          data_splitmp=c(data_splitmp,sampleneed)
        }
      }else{
        datatake=unlist(split(datatmp$IDnum,datatmp$Group)[maxg])
        data_splitmp=sample(datatake,need)
        datatmp=datatmp[-which(datatmp$IDnum%in%data_splitmp==TRUE),]
      }
      if(Z2_q%%ng==0){
        data_splitZ[[j+1]]=c(data_splitmp,data_splitmp2)
      }else{
        data_splitZ[[j+1]]=c(samrest,data_splitmp,data_splitmp2)
      }
      dataNC$SetNum[data_splitZ[[j+1]]]=paste("Set",j+1,sep="")
      setname=c(paste("Set",j+1,sep=""),setname)
    }
